...
class PuzzleEventEditorApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Puzzle Event Editor")
        self.characters = fix_data_io.load_characters(PROJECT_ROOT)
        
        # 1. Load Levels
        self.levels = puzzle_levels_io.load_levels(PROJECT_ROOT)
        self.original_levels = copy.deepcopy(self.levels)
        
        # 2. Load existing puzzle events to memory
        self.puzzle_events = {e.id: e for e in fix_data_io.load_all_puzzle_events(PROJECT_ROOT)}
        self.original_events = copy.deepcopy(self.puzzle_events)
        
        self.current_level_idx = None
        self.current_tab = "intro" # "intro" or "outro"
    ...
    def _setup_ui(self):
        ...
        ttk.Label(left_frame, text="Puzzle Levels").pack(anchor="w")
        self.lv_tree = ttk.Treeview(left_frame, columns=("title", "intro", "outro"), show="headings", selectmode="browse")
        self.lv_tree.heading("title", text="Level Title")
        self.lv_tree.heading("intro", text="Intro Event")
        self.lv_tree.heading("outro", text="Outro Event")
        self.lv_tree.pack(fill=tk.BOTH, expand=True)
        self.lv_tree.bind("<<TreeviewSelect>>", self._on_level_select)
        ...
        # Tab selection (Intro / Outro)
        tab_frame = ttk.Frame(right_frame)
        tab_frame.pack(fill=tk.X, pady=4)
        
        self.var_tab = tk.StringVar(value="intro")
        ttk.Radiobutton(tab_frame, text="Intro Event", variable=self.var_tab, value="intro", command=self._on_tab_changed).pack(side=tk.LEFT, padx=4)
        ttk.Radiobutton(tab_frame, text="Outro Event", variable=self.var_tab, value="outro", command=self._on_tab_changed).pack(side=tk.LEFT, padx=4)
        ...
        ttk.Button(btn_f2, text="Create Event", command=self._create_event).pack(side=tk.LEFT)
        ttk.Button(btn_f2, text="Add Line", command=self._add_line).pack(side=tk.LEFT, padx=(10,0))
        ...
        ttk.Label(edit_form, text="SE:").grid(row=2, column=3, padx=4, pady=4, sticky="e")
    ...
    def _get_current_event_id(self) -> str:
        if self.current_level_idx is None: return ""
        lv = self.levels[self.current_level_idx]
        return lv.intro_event_id if self.current_tab == "intro" else lv.outro_event_id

    def _get_current_event(self) -> fix_data_io.EventEntry:
        eid = self._get_current_event_id()
        if not eid: return None
        return self.puzzle_events.get(eid, None)

    def _refresh_level_list(self):
        for item in self.lv_tree.get_children():
            self.lv_tree.delete(item)
        for i, lv in enumerate(self.levels):
            self.lv_tree.insert("", tk.END, iid=str(i), values=(lv.title, lv.intro_event_id, lv.outro_event_id))

    def _on_level_select(self, event):
        sel = self.lv_tree.selection()
        if sel:
            self.current_level_idx = int(sel[0])
        else:
            self.current_level_idx = None
        self._refresh_line_list()

    def _on_tab_changed(self):
        self.current_tab = self.var_tab.get()
        self._refresh_line_list()
    ...
    def _refresh_line_list(self, select_idx=None):
        ...
        if not eid:
            self.lbl_lines.config(text="Lines: No event assigned")
        elif not evt:
            self.lbl_lines.config(text=f"Lines: Missing event {eid}")
        else:
            self.lbl_lines.config(text=f"Lines: {eid}")
            lines = evt.lines
    ...
    def _create_event(self):
        if self.current_level_idx is None: return
        lv = self.levels[self.current_level_idx]
        eid = f"PZE_LV{self.current_level_idx}_{self.current_tab.upper()}"
        if eid in self.puzzle_events:
            messagebox.showwarning("Warning", f"Event {eid} already exists.")
            return
        
        self.puzzle_events[eid] = fix_data_io.EventEntry(id=eid, title_ja=f"LV{self.current_level_idx} {self.current_tab}", lines=[])
        if self.current_tab == "intro":
            lv.intro_event_id = eid
        else:
            lv.outro_event_id = eid
            
        self._refresh_level_list()
        self.lv_tree.selection_set(str(self.current_level_idx))
        self._refresh_line_list()
    ...
    def _is_dirty(self):
        lv1 = [dataclasses.asdict(lv) for lv in self.levels]
        lv2 = [dataclasses.asdict(lv) for lv in self.original_levels]
        if shell.snapshot_json(lv1) != shell.snapshot_json(lv2):
            return True
        
        ev1 = {k: dataclasses.asdict(v) for k, v in self.puzzle_events.items()}
        ev2 = {k: dataclasses.asdict(v) for k, v in self.original_events.items()}
        return shell.snapshot_json(ev1) != shell.snapshot_json(ev2)

    def _save_all(self):
        # 1. Save levels
        puzzle_levels_io.save_levels(PROJECT_ROOT, self.levels)
        self.original_levels = copy.deepcopy(self.levels)
        
        # 2. Save puzzle events
        for e in self.puzzle_events.values():
            filename = fix_data_io.puzzle_event_filename_from_id(e.id)
            fix_data_io.save_puzzle_event_file(PROJECT_ROOT, filename, e)
        self.original_events = copy.deepcopy(self.puzzle_events)
        
        self._refresh_level_list()
        messagebox.showinfo("Saved", "Levels and Puzzle Events saved successfully.")
